...
4. Data structures are used in this script. Primitive data structures used include 'int' and 'char'. 
The non-primitive data structure, 'lexeme[]', is a one dimensional array of characters used to store the current lexeme being analyzed.
In addition, file pointers, 'in_fp' and 'out_fp', are also used to acces the input and output files.
...
int indent = 0; /* For indentation in output parse tree */

/***********************************/
/* MAIN DRIVER */
int main() {
    /* open the input data file and proces contents */
...
void getChar() {
    ...
    if (nextChar != EOF) {
        // If nextChar is letter = LETTER charClass
        if (isalpha(nextChar))
            charClass = LETTER;
        // If nextChar is number = DIGIT charClass
        else if (isdigit(nextChar))
            charClass = DIGIT;
        // If nextChar is something else = UNKNOWN charClass
        else
            charClass = UNKNOWN;
    } else {
        charClass = EOF;
...
/* factor():Parses strings in the lnaguage generated by the rule:
